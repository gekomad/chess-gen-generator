...
//    println("----")
//    assert(tmp_struct.MASK_BIT_SET_NOBOUND_TMP(0)(0) == 140174251707552l)
//    (0 until 64).foreach { i =>
//      (0 until 64).foreach { j =>
//        print(tmp_struct.MASK_BIT_SET_NOBOUND_TMP(i)(j) + ",")
//      }
//    }
//    println("----")
    def rankIdx(position: Int, allpieces: Long): Char = ((allpieces >>> RANK_ATX8(position)) & 0xff).toChar

    def fileIdx(position: Int, allpieces: Long): Char = ((((allpieces & FILE_(position)) * MAGIC_KEY_FILE_RANK) >>> 56) & 0xff).toChar

    def diagonalIdx(position: Int, allpieces: Long): Char = ((((allpieces & DIAGONAL(position)) * MAGIC_KEY_DIAG_ANTIDIAG) >>> 56) & 0xff).toChar

    def antiDiagonalIdx(position: Int, allpieces: Long): Char = ((((allpieces & ANTIDIAGONAL(position)) * MAGIC_KEY_DIAG_ANTIDIAG) >>> 56) & 0xff).toChar
    ...
    def performDiagShift(position: Int, allpieces: Long): Long = {
      ...
      val k: Long  = if (q1 != 0) tmp_struct.MASK_BIT_SET_NOBOUND_TMP(position)(BITScanReverse(q1)) else MASK_BIT_SET_LEFT_LOWER(position)
      val q2       = allpieces & MASK_BIT_UNSET_LEFT_DOWN(position)
      k | (if (q2 != 0) tmp_struct.MASK_BIT_SET_NOBOUND_TMP(position)(BITScanForward(q2)) else MASK_BIT_SET_LEFT_UPPER(position))
    }

    def performColumnShift(position: Int, allpieces: Long): Long = {
      val q1: Long = allpieces & MASK_BIT_UNSET_UP(position)
      val k: Long  = if (q1 != 0) tmp_struct.MASK_BIT_SET_NOBOUND_TMP(position)(BITScanReverse(q1)) else MASK_BIT_SET_VERT_LOWER(position)
      val q2       = allpieces & MASK_BIT_UNSET_DOWN(position)
      k | (if (q2 != 0) tmp_struct.MASK_BIT_SET_NOBOUND_TMP(position)(BITScanForward(q2)) else MASK_BIT_SET_VERT_UPPER(position))

    }

    def performRankShift(position: Int, allpieces: Long): Long = {
      val q1: Long = allpieces & MASK_BIT_UNSET_RIGHT(position)
      val k: Long  = if (q1 != 0L) tmp_struct.MASK_BIT_SET_NOBOUND_TMP(position)(BITScanForward(q1)) else MASK_BIT_SET_ORIZ_LEFT(position)
      val q2       = allpieces & MASK_BIT_UNSET_LEFT(position)
      k | (if (q2 != 0L) tmp_struct.MASK_BIT_SET_NOBOUND_TMP(position)(BITScanReverse(q2)) else MASK_BIT_SET_ORIZ_RIGHT(position))

    }

    def performAntiDiagShift(position: Int, allpieces: Long): Long = {
      val q1: Long = allpieces & MASK_BIT_UNSET_RIGHT_UP(position)
      val k: Long  = if (q1 != 0) tmp_struct.MASK_BIT_SET_NOBOUND_TMP(position)(BITScanReverse(q1)) else MASK_BIT_SET_RIGHT_LOWER(position)
      val q2: Long = allpieces & MASK_BIT_UNSET_RIGHT_DOWN(position)
      k | (if (q2 != 0) tmp_struct.MASK_BIT_SET_NOBOUND_TMP(position)(BITScanForward(q2)) else MASK_BIT_SET_RIGHT_UPPER(position))
    }
    ...
    (0 until 64).foreach { pos =>
      getCombination(DIAGONAL(pos)).foreach { allpieces =>
        BITBOARD_DIAGONAL(pos)(diagonalIdx(pos, allpieces)) = performDiagShift(pos, allpieces) | performDiagCapture(pos, allpieces)
      }
    }

    (0 until 64).foreach { pos =>
      getCombination(FILE_(pos)).foreach { allpieces =>
        BITBOARD_FILE(pos)(fileIdx(pos, allpieces)) = performColumnShift(pos, allpieces) | performColumnCapture(pos, allpieces)
      }
    }

    (0 until 64).foreach { pos =>
      getCombination(RANK(pos)).foreach { allpieces =>
        BITBOARD_RANK(pos)(rankIdx(pos, allpieces)) = performRankShift(pos, allpieces) | performRankCapture(pos, allpieces)
      }
    }

    (0 until 64).foreach { pos =>
      getCombination(ANTIDIAGONAL(pos)).foreach { allpieces =>
        BITBOARD_ANTIDIAGONAL(pos)(antiDiagonalIdx(pos, allpieces)) = performAntiDiagShift(pos, allpieces) | performAntiDiagCapture(pos, allpieces)
      }
    }
