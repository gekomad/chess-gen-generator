...
case class ChessBoard private () {
  ...
  val bitboards: Bitboards.Bitboard = Bitboards()
  val chessboard: Array[BitmapPosition] = Array.ofDim[Long](16)

  def getBitmapNoPawns(side: Side): Long =
    chessboard(ROOK_BLACK + side) | chessboard(BISHOP_BLACK + side) | chessboard(KNIGHT_BLACK + side) | chessboard(KING_BLACK + side) | chessboard(QUEEN_BLACK + side)

  def zobristKey: BitmapPosition = chessboard(ZOBRISTKEY_IDX)

  def getNpiecesNoPawnNoKing(side: Side): Int =
    bitCount(chessboard(ROOK_BLACK + side) | chessboard(BISHOP_BLACK + side) | chessboard(KNIGHT_BLACK + side) | chessboard(QUEEN_BLACK + side))

  def getBitmap(side: Side): Long =
    chessboard(PAWN_BLACK + side) | chessboard(ROOK_BLACK + side) | chessboard(BISHOP_BLACK + side) | chessboard(KNIGHT_BLACK + side) | chessboard(KING_BLACK + side) | chessboard(QUEEN_BLACK + side)
  ...
  def updateZobristKey(piece: Int, position: Position): Unit = chessboard(ZOBRISTKEY_IDX) ^= Random.RANDOM_KEY(piece)(position)

  private def loadFen(fen: String): Side = {
    ...
    chessboard(SIDETOMOVE_IDX).toInt
  }
  ...
  def display(): Unit = {
    ...
    (0 until 64).foreach { t =>
      ...
      if (t % 8 == 0) {
        print("\n   ----+---+---+---+---+---+---+----\n")
        print((8 - RANK_AT(t)).toString + " | ")
      }
      val o = FEN_PIECE(getPieceAt(WHITE, POW2(63 - t)))

      var k = ' '
      if (o != '-') { k = o } else { k = FEN_PIECE(getPieceAt(BLACK, POW2(63 - t))) }
      if (k == '-') { x = ' ' } else { x = k }
    ...
    println("en passant position: " + (if (chessboard(ENPASSANT_IDX) == NO_ENPASSANT) "None" else chessboard(ENPASSANT_IDX)))
  ...
  def boardToFen: String = {
    ...
    if (chessboard(ENPASSANT_IDX) == NO_ENPASSANT) {
      ...
      fen += " "
      fen += (if (chessboard(SIDETOMOVE_IDX) != 0) BOARD(chessboard(ENPASSANT_IDX).toInt + 8) else BOARD(chessboard(ENPASSANT_IDX).toInt - 8))
...
object ChessBoard {
  ...
  private def getPieceByChar(c: Char): Int = (0 until 12).find(c == FEN_PIECE(_)).getOrElse(-1)
