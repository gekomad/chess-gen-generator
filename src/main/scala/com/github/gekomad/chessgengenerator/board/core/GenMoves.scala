...
import com.github.gekomad.chessgengenerator.board.core.Def._
import com.github.gekomad.chessgengenerator.board.core.ChessBoard.{BitmapPosition, Position, Side}
import com.github.gekomad.chessgengenerator.board.core.GenMoves.{_Tmove, _TmoveP}
...
object GenMoves {
  ...
  trait _Tmove {
    ...
    override def toString: String = {
      ...
      s"_Tmove (promotionPiece: ${ChessBoard.decodePiece(promotionPiece.toInt)}, pieceFrom: ${ChessBoard.decodePiece(pieceFrom.toInt)}," +
        s"capturedPiece: ${ChessBoard.decodePiece(capturedPiece.toInt)}, from: $f, to: $t, side: ${ChessBoard.decodeSide(side.toLong)}, " +
        s"type1: ${type1.toInt} ${ChessBoard.decodeType(type1.toInt)}, score: $score)"
    }
  }

  def apply(fen: String, perftMode: Boolean): Option[GenMoves] = ChessBoard(fen).map(new GenMoves(_, perftMode))
}
...
//  case class MoveIterator(list: _TmoveP) extends AbstractIterator[_Tmove] {
//    private var count: Int        = 0
//    override def hasNext: Boolean = count >= list.size
//    override def next(): _Tmove =
//      if (hasNext) {
//        count += 1
//        list.moveList(count - 1)
//      } else throw new NoSuchElementException("next on empty iterator")
//  }

//private def iterator(listId: Int) = MoveIterator(genList(listId))
  ...
  private def getDiagShiftAndCapture(position: Position, enemies: Long, allpieces: Long): Long = {
    assert(position >= 0 && position < 64)
    val nuovo: Long = chessBoard.bitboards.getDiagonalAntiDiagonal(position, allpieces)
    (nuovo & enemies) | (nuovo & ~allpieces)
  }

  private def getDiagShiftCount(position: Position, allpieces: Long): Int = {
    assert(position >= 0 && position < 64)
    bitCount(chessBoard.bitboards.getDiagonalAntiDiagonal(position, allpieces) & ~allpieces)
  }
  ...
  private def performPawnCapture(listId: Int, side: Side, enemies: Long): Boolean = {
    ...
    var x: Long = -1L
    val GG: Int = if (side != 0) {
      x = (chessBoard.chessboard(side) << 7) & TABCAPTUREPAWN_LEFT & enemies
      -7
    } else {
      x = (chessBoard.chessboard(side) >> 7) & TABCAPTUREPAWN_RIGHT & enemies
      7
    }
    while (x != 0) {
      val o = BITScanForward(x)
      if ((side != 0 && o > 55) || (side == 0 && o < 8)) { //PROMOTION
        if (pushmove(listId, PROMOTION_MOVE_MASK, o + GG, o, side, QUEEN_BLACK + side, side)) {
          return true //queen
        }
        if (perftMode) {
          if (pushmove(listId, PROMOTION_MOVE_MASK, o + GG, o, side, KNIGHT_BLACK + side, side)) {
            return true //knight
          }
          if (pushmove(listId, PROMOTION_MOVE_MASK, o + GG, o, side, ROOK_BLACK + side, side)) {
            return true //rock
          }
          if (pushmove(listId, PROMOTION_MOVE_MASK, o + GG, o, side, BISHOP_BLACK + side, side)) {
            return true //bishop
          }
        }
      } else if (pushmove(listId, STANDARD_MOVE_MASK, o + GG, o, side, NO_PROMOTION, side)) {
        return true
      }
      x = resetLSB(x)
    }
    val GGG = if (side != 0) {
      x = (chessBoard.chessboard(side) << 9) & TABCAPTUREPAWN_RIGHT & enemies
      -9
    } else {
      x = (chessBoard.chessboard(side) >> 9) & TABCAPTUREPAWN_LEFT & enemies
      9
    }
    while (x != 0) {
      val o = BITScanForward(x)
      if ((side != 0 && o > 55) || (side == 0 && o < 8)) { //PROMOTION
        if (pushmove(listId, PROMOTION_MOVE_MASK, o + GGG, o, side, QUEEN_BLACK + side, side)) {
          return true //queen
        }
        if (perftMode) {
          if (pushmove(listId, PROMOTION_MOVE_MASK, o + GGG, o, side, KNIGHT_BLACK + side, side)) {
            return true //knight
          }
          if (pushmove(listId, PROMOTION_MOVE_MASK, o + GGG, o, side, BISHOP_BLACK + side, side)) {
            return true
          }
          if (pushmove(listId, PROMOTION_MOVE_MASK, o + GGG, o, side, ROOK_BLACK + side, side)) {
            return true //rock
          }
        }
      } else if (pushmove(listId, STANDARD_MOVE_MASK, o + GGG, o, side, NO_PROMOTION, side)) {
        return true
      }
      x = resetLSB(x)
    }
    //ENPASSANT
    if (chessBoard.chessboard(ENPASSANT_IDX) != NO_ENPASSANT) {
      x = ENPASSANT_MASK(side ^ 1)(chessBoard.chessboard(ENPASSANT_IDX).toInt) & chessBoard.chessboard(side)
      @tailrec
      def go(x: Long): Unit = if (x != 0) {
        val o = BITScanForward(x)
        val k = (if (side != 0) chessBoard.chessboard(ENPASSANT_IDX) + 8 else chessBoard.chessboard(ENPASSANT_IDX) - 8).toInt
  ...
  private def isAttacked(side: Side, position: Position, allpieces: Long): Boolean = getAttackers(side, exitOnFirst = true, position, allpieces) != 0

  private def getAllAttackers(side: Side, position: Position, allpieces: Long): Long = getAttackers(side, exitOnFirst = false, position, allpieces)

  private def inCheck(side: Side, type1: Char, from: Position, to: Position, pieceFrom: Position, pieceTo: Position, promotionPiece: Int): Boolean = {
    ...
    val result1 = type1 & 0x3 match {
      ...
      case STANDARD_MOVE_MASK =>
        ...
        val result = isAttacked(side, BITScanForward(chessBoard.chessboard(KING_BLACK + side)), chessBoard.getBitmap(BLACK) | chessBoard.getBitmap(WHITE))
        chessBoard.chessboard(pieceFrom) = from1
        if (pieceTo != SQUARE_FREE) chessBoard.chessboard(pieceTo) = to1
        result
      case PROMOTION_MOVE_MASK =>
        val to1   = if (pieceTo != SQUARE_FREE) chessBoard.chessboard(pieceTo) else 0L
        val from1 = chessBoard.chessboard(pieceFrom)
        val p1    = chessBoard.chessboard(promotionPiece)
        chessBoard.chessboard(pieceFrom) &= NOTPOW2(from)
        if (pieceTo != SQUARE_FREE) chessBoard.chessboard(pieceTo) &= NOTPOW2(to)
  ...
  private def pushmove(listId: Int, type1: Char, from: Position, to: Position, side: Side, promotionPiece: Int, pieceFrom: Int): Boolean = {
//    assert(from >= 0 && from < 64, type1.toInt)
    assert(chessBoard.chessboard(KING_BLACK) != 0)
    assert(chessBoard.chessboard(KING_WHITE) != 0)
    var piece_captured = SQUARE_FREE
    var res            = false
    if (((type1 & 0x3) != ENPASSANT_MOVE_MASK) && 0 == (type1 & 0xc)) {
      piece_captured = if (side != 0) chessBoard.getPieceAt(BLACK, POW2(to)) else chessBoard.getPieceAt(WHITE, POW2(to))
    ...
    if ((type1 & 0xc) == 0 && (forceCheck || perftMode)) { //no castle
    ...
    if ((type1 & 0x3) != 0) {
      ...
      if (!perftMode) {
        if (res) {
          ...
          mos.score += (if (PIECES_VALUE(piece_captured) >= PIECES_VALUE(pieceFrom)) (PIECES_VALUE(piece_captured) - PIECES_VALUE(pieceFrom)) * 2 else PIECES_VALUE(piece_captured))
  ...
  private def inCheck(side: Side): Boolean =
    isAttacked(side, BITScanForward(chessBoard.chessboard(KING_BLACK + side)), chessBoard.getBitmap(BLACK) | chessBoard.getBitmap(WHITE))
  ...
  private def performRankFileCapture(listId: Int, piece: Int, enemies: Long, side: Int, allpieces: Long): Boolean = {
    ...
    def w0(x2: Long): Boolean =
      if (x2 == 0) false
        ...
        if (w(chessBoard.bitboards.getRankFile(position, allpieces) & enemies)) true
        else
          w0(resetLSB(x2))
      }
  ...
  private def performDiagCapture(listId: Int, piece: Int, enemies: Long, side: Side, allpieces: Long): Boolean = {
    ...
    def w2(x2: Long): Boolean =
      if (x2 == 0) false
        ...
        if (w(chessBoard.bitboards.getDiagonalAntiDiagonal(position, allpieces) & enemies)) true
        else
          w2(resetLSB(x2))
      }
  ...
  private def performRankFileShift(listId: Int, piece: Int, side: Side, allpieces: Long): Unit = {
    ...
    def go(x2: Long): Unit = if (x2 != 0) {
      val position = BITScanForward(x2)
      val rankFile = chessBoard.bitboards.getRankFile(position, allpieces) & ~allpieces
      @tailrec
  ...
  private def performDiagShift(listId: Int, piece: Int, side: Side, allpieces: Long): Unit = {
    ...
    def go(x2: Long): Unit = if (x2 != 0) {
      ...
      val position = BITScanForward(x2)
      val diag     = chessBoard.bitboards.getDiagonalAntiDiagonal(position, allpieces) & ~allpieces
  ...
  private def generateMoves(listId: Int, piece: Int, side: Side, allpieces: Long): Unit = {
    assert(side == 0 || side == 1)
    if (side != 0) generateMoves(listId, WHITE, allpieces) else generateMoves(listId, BLACK, allpieces)
  }
  ...
  private def getMobilityPawns(side: Side, ep: Int, ped_friends: Long, enemies: Long, xallpieces: Long): Int = {
    ...
    if (ep == NO_ENPASSANT) 0
      ...
      bitCount((ped_friends << 8) & xallpieces) + bitCount(((((ped_friends & TABJUMPPAWN) << 8) & xallpieces) << 8) & xallpieces) + bitCount(
        (chessBoard.chessboard(side) << 7) & TABCAPTUREPAWN_LEFT & enemies
      ) + bitCount((chessBoard.chessboard(side) << 9) & TABCAPTUREPAWN_RIGHT & enemies)
    else
      bitCount((ped_friends >> 8) & xallpieces) + bitCount(((((ped_friends & TABJUMPPAWN) >> 8) & xallpieces) >> 8) & xallpieces) + bitCount(
        (chessBoard.chessboard(side) >> 7) & TABCAPTUREPAWN_RIGHT & enemies
      ) + bitCount((chessBoard.chessboard(side) >> 9) & TABCAPTUREPAWN_LEFT & enemies)
  }
  ...
  private def clearKillerHeuristic(): Unit = killerHeuristic.foreach(a => util.Arrays.fill(a, 0))

//private def getNextMove(moveP: _TmoveP): Option[_Tmove] = {
//    val genList1  = moveP.moveList
//    val listcount = moveP.size
//    var bestId    = -1
//    var bestScore = -1
//    var jj        = -1
//    (0 until listcount)
...
//        if (!genList1(j).used) {
//          bestId = j
//          bestScore = genList1(bestId).score
//
//        }
//      }
//
...
//      None
//    } else {
//      (jj + 1 until listcount).foreach { i =>
//        if (!genList1(i).used && genList1(i).score > bestScore) {
//          bestId = i
//          bestScore = genList1(bestId).score
//        }
//      }
//      genList1(bestId).used = true
//      Some(genList1(bestId))
//    }
//  }
  ...
  private def getMobilityCastle(side: Side, allpieces: Long): Int = {
    ...
    var count = 0
    if (side == WHITE) {
      if ((POW2_3 & chessBoard.chessboard(KING_WHITE)) != 0 && ((allpieces & 0x6L) == 0) && (chessBoard.chessboard(RIGHT_CASTLE_IDX) & RIGHT_KING_CASTLE_WHITE_MASK) != 0
          && (chessBoard.chessboard(ROOK_WHITE) & POW2_0) != 0 && !isAttacked(WHITE, 1, allpieces) && !isAttacked(WHITE, 2, allpieces) && !isAttacked(WHITE, 3, allpieces)) {
        count += 1
      }
      if ((POW2_3 & chessBoard.chessboard(KING_WHITE)) != 0 && ((allpieces & 0x70L) == 0) && (chessBoard.chessboard(RIGHT_CASTLE_IDX) & RIGHT_QUEEN_CASTLE_WHITE_MASK) != 0
          && (chessBoard.chessboard(ROOK_WHITE) & POW2_7) != 0 && !isAttacked(WHITE, 3, allpieces) && !isAttacked(WHITE, 4, allpieces) && !isAttacked(WHITE, 5, allpieces)) {
        count += 1
      }
    } else {
      if ((POW2_59 & chessBoard.chessboard(KING_BLACK)) != 0 && (chessBoard.chessboard(RIGHT_CASTLE_IDX) & RIGHT_KING_CASTLE_BLACK_MASK) != 0 && ((allpieces & 0x600000000000000L) == 0)
          && (chessBoard.chessboard(ROOK_BLACK) & POW2_56) != 0 && !isAttacked(BLACK, 57, allpieces) && !isAttacked(BLACK, 58, allpieces) && !isAttacked(BLACK, 59, allpieces)) {
        count += 1
      }
      if ((POW2_59 & chessBoard.chessboard(KING_BLACK)) != 0 && (chessBoard.chessboard(RIGHT_CASTLE_IDX) & RIGHT_QUEEN_CASTLE_BLACK_MASK) != 0 && ((allpieces & 0x7000000000000000L) == 0)
          && (chessBoard.chessboard(ROOK_BLACK) & POW2_63) != 0 && !isAttacked(BLACK, 59, allpieces) && !isAttacked(BLACK, 60, allpieces) && !isAttacked(BLACK, 61, allpieces)) {
        count += 1
      }
    }
    count
  ...
  private def tryAllCastle(listId: Int, side: Side, allpieces: Long): Unit = {
    ...
    assert(side == 0 || side == 1)
    if (side == WHITE) {
      if ((POW2_3 & chessBoard.chessboard(KING_WHITE)) != 0 && 0 == (allpieces & 0x6L) && (chessBoard.chessboard(RIGHT_CASTLE_IDX) & RIGHT_KING_CASTLE_WHITE_MASK) != 0
          && (chessBoard.chessboard(ROOK_WHITE) & POW2_0) != 0 && !isAttacked(WHITE, 1, allpieces) && !isAttacked(WHITE, 2, allpieces) &&
          !isAttacked(WHITE, 3, allpieces)) {
        pushmove(listId, KING_SIDE_CASTLE_MOVE_MASK, -1, -1, WHITE, NO_PROMOTION, -1)
        ()
      }
      if ((POW2_3 & chessBoard.chessboard(KING_WHITE)) != 0 && 0 == (allpieces & 0x70L) && (chessBoard.chessboard(RIGHT_CASTLE_IDX) & RIGHT_QUEEN_CASTLE_WHITE_MASK) != 0
          && (chessBoard.chessboard(ROOK_WHITE) & POW2_7) != 0 && !isAttacked(WHITE, 3, allpieces) && !isAttacked(WHITE, 4, allpieces) &&
          !isAttacked(WHITE, 5, allpieces)) {
        pushmove(listId, QUEEN_SIDE_CASTLE_MOVE_MASK, -1, -1, WHITE, NO_PROMOTION, -1)
        ()
      }
    } else {
      if ((POW2_59 & chessBoard.chessboard(KING_BLACK)) != 0 && (chessBoard.chessboard(RIGHT_CASTLE_IDX) & RIGHT_KING_CASTLE_BLACK_MASK) != 0 &&
          0 == (allpieces & 0x600000000000000L) && (chessBoard.chessboard(ROOK_BLACK) & POW2_56) != 0 && !isAttacked(BLACK, 57, allpieces) &&
      ...
      if ((POW2_59 & chessBoard.chessboard(KING_BLACK)) != 0 && (chessBoard.chessboard(RIGHT_CASTLE_IDX) & RIGHT_QUEEN_CASTLE_BLACK_MASK) != 0
          && 0 == (allpieces & 0x7000000000000000L) && (chessBoard.chessboard(ROOK_BLACK) & POW2_63) != 0 && !isAttacked(BLACK, 59, allpieces)
...
//  private def makeZobristKey(): Unit = chessBoard.makeZobristKey()
...
//    var posTo       = SQUARE_FREE
//    var posFrom     = SQUARE_FREE
    var movecapture = ' '
...
//    val bound = 0
    ...
    if (exitOnFirst && attackers != 0) return 1
    ///king
    ...
    if (exitOnFirst && attackers != 0) return 1
    ///bishop queen
    var enemies = chessBoard.chessboard(BISHOP_BLACK + (side ^ 1)) | chessBoard.chessboard(QUEEN_BLACK + (side ^ 1))
    var nuovo   = chessBoard.bitboards.getDiagonalAntiDiagonal(position, allpieces) & enemies
    while (nuovo != 0) {
      val bound = BITScanForward(nuovo)
      attackers |= POW2(bound)
      if (exitOnFirst && attackers != 0) return 1
      nuovo = resetLSB(nuovo)
    }
    enemies = chessBoard.chessboard(ROOK_BLACK + (side ^ 1)) | chessBoard.chessboard(QUEEN_BLACK + (side ^ 1))
    nuovo = chessBoard.bitboards.getRankFile(position, allpieces) & enemies
    @tailrec
